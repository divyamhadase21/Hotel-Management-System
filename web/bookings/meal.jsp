...
<html>
<head>
    <title>Meal Order — Luxury Stay</title>
    ...
    <style>
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        input, select { width:100%; padding:11px 16px; border-radius:10px; border:1px solid var(--glass-border); background:rgba(255,255,255,0.07); color:var(--text-primary); font-size:14px; font-family:'Inter',sans-serif; outline:none; transition:border-color .2s; }
        input:focus, select:focus { border-color:var(--gold); }
        select option { background:#0C1B33; }
        .meal-grid { display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:18px; }
        .meal-option { display:flex; align-items:center; gap:8px; padding:10px 14px; border-radius:10px; border:1px solid var(--glass-border); background:rgba(255,255,255,0.05); cursor:pointer; transition:border-color .2s,background .2s; }
        .meal-option:hover { border-color:var(--gold); background:rgba(201,168,76,0.1); }
        .meal-option input[type="checkbox"] { width:16px; height:16px; accent-color:var(--gold); }
        .meal-option span { color:var(--text-primary); font-size:14px; }
        ...
        .section-label { color:var(--text-muted); font-size:13px; font-weight:500; margin-bottom:10px; display:block; }
    </style>
</head>
<body>
    <nav class="navbar">
    ...
    <div class="container">
        <div class="card">
            ...
            <form action="../MealServlet" method="post">
                ...
                <div class="form-group">
                    <span class="section-label">Select Items</span>
                    <div class="meal-grid">
                        <label class="meal-option"><input type="checkbox" name="items" value="Paneer Butter Masala"/><span>🧀 Paneer Masala</span></label>
                        <label class="meal-option"><input type="checkbox" name="items" value="Dal Tadka"/><span>🍲 Dal Tadka</span></label>
                        <label class="meal-option"><input type="checkbox" name="items" value="Biryani"/><span>🍛 Biryani</span></label>
                        <label class="meal-option"><input type="checkbox" name="items" value="Roti"/><span>🫓 Roti</span></label>
                        <label class="meal-option"><input type="checkbox" name="items" value="Salad"/><span>🥗 Salad</span></label>
                        <label class="meal-option"><input type="checkbox" name="items" value="Dessert"/><span>🍮 Dessert</span></label>
                    </div>
                </div>
                <div class="form-group">
